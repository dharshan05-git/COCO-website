...
function ReaderPage({ books, difficultWords, settings, onUpdateBook, onAddWord, onFinish, language, setLanguage, profile, onReset }: { books: Book[]; difficultWords: DifficultWord[]; settings: Settings; onUpdateBook: (id: string, progress: number) => void; onAddWord: (word: DifficultWord) => void; onFinish: (session: ReadingSession) => void; language: Language; setLanguage: (l: Language) => void; profile: Profile; onReset: () => void }) {
  ...
  return <AppShell profile={profile} language={language} setLanguage={setLanguage} onReset={onReset}><div className="mx-auto max-w-7xl px-5 py-6 sm:px-10 lg:px-14 lg:py-9">
    ...
    <div className="grid items-start gap-7 xl:grid-cols-[minmax(0,1fr)_310px]"><article className={`paper-card relative rounded-[28px] p-6 sm:p-10 lg:p-14 ${settings.dyslexiaFont ? "tracking-[.025em]" : ""}`} style={{ fontSize: `${settings.textSize}px`, lineHeight: settings.lineSpacing }}>
       <div className="mb-10 flex items-start justify-between border-b border-[hsl(var(--border))] pb-7"><div><p className="font-mono-coco text-[10px] uppercase tracking-[.2em] text-[hsl(var(--primary))]">{book.format} / {book.author}</p><h1 className="mt-3 font-display text-4xl leading-none sm:text-5xl">{book.title}</h1><p className="mt-3 text-sm text-[hsl(var(--muted-foreground))]">Listen to the whole lesson in a calm, steady voice.</p></div><div className="flex items-center gap-2">{(isSpeaking || isPaused) && <button type="button" onClick={togglePause} aria-label={isPaused ? "Resume lesson audio" : "Pause lesson audio"} data-testid="button-pause-lesson" className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[hsl(var(--accent))] text-[hsl(var(--foreground))] hover:opacity-90">{isPaused ? <Play size={20} /> : <Pause size={20} />}</button>}<button type="button" onClick={speakLesson} aria-label="Listen to whole lesson" data-testid="button-speak-lesson" className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[hsl(var(--secondary))] text-[hsl(var(--primary))] hover:bg-[hsl(var(--accent))]"><Volume2 size={21} /></button></div></div>
      ...
      <div className="mt-12 border-t border-[hsl(var(--border))] pt-6"><div className="mb-3 flex justify-between text-xs font-bold text-[hsl(var(--muted-foreground))]"><span>Reading trail</span><span>{Math.round(((currentIndex + 1) / Math.max(1, wordIndexes.length)) * 100)}%</span></div><ProgressBar value={((currentIndex + 1) / Math.max(1, wordIndexes.length)) * 100} accent /><div className="mt-6 flex flex-wrap items-center gap-3"><button type="button" onClick={() => { const next = Math.min(wordIndexes.length - 1, currentIndex + 1); setCurrentIndex(next); onUpdateBook(book.id, Math.round(((next + 1) / Math.max(1, wordIndexes.length)) * 100)); }} data-testid="button-next-word" className="coco-button flex items-center gap-2 rounded-2xl bg-[hsl(var(--primary))] px-4 py-3 text-sm font-bold text-[hsl(var(--primary-foreground))]">{t("next")} <ChevronRight size={16} /></button><button type="button" onClick={() => speak(selectedWord || tokens[currentTokenIndex] || "")} data-testid="button-listen-word" className="flex items-center gap-2 rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-4 py-3 text-sm font-bold shadow-sm transition hover:border-[hsl(var(--primary))]"><Headphones size={16} /> {isSpeaking && !isPaused ? "Playing…" : t("listen")}</button><button type="button" onClick={togglePause} disabled={!isSpeaking && !isPaused} data-testid="button-pause-audio" className={`flex items-center gap-2 rounded-2xl border px-4 py-3 text-sm font-bold transition ${isPaused ? "border-[hsl(var(--primary))] bg-[hsl(var(--accent))] text-[hsl(var(--foreground))]" : isSpeaking ? "border-[hsl(var(--primary))] bg-[hsl(var(--secondary))] text-[hsl(var(--foreground))]" : "border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--muted-foreground))] opacity-50 cursor-not-allowed"}`}>{isPaused ? <Play size={16} /> : <Pause size={16} />} {isPaused ? t("resume") : t("pause")}</button>{!done && <button type="button" onClick={finish} data-testid="button-finish-mission" className="ml-auto flex items-center gap-2 rounded-2xl bg-[hsl(var(--accent))] px-4 py-3 text-sm font-bold"><Check size={16} /> {t("finish")}</button>}</div></div>
    </article>
    <aside className="space-y-5"><div className="paper-card rounded-[24px] p-5"><div className="mb-4 flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-[hsl(var(--accent))]"><Brain size={19} /></div><div><h2 className="font-bold">{t("soundSteps")}</h2><p className="text-xs text-[hsl(var(--muted-foreground))]">{t("tapWord")}</p></div></div>{selectedWord ? <div className="rounded-2xl bg-[hsl(var(--secondary))] p-4" data-testid="panel-word-support"><p className="font-display text-3xl">{selectedWord}</p><div className="mt-3 flex flex-wrap gap-2">{chunkWord(selectedWord).map((chunk, index) => <span key={`${chunk}-${index}`} className="rounded-lg bg-[hsl(var(--card))] px-2 py-1 font-mono-coco text-sm font-medium shadow-sm">{chunk}</span>)}</div><div className="mt-4 flex items-center gap-2"><button type="button" onClick={() => speak(selectedWord)} data-testid="button-pronounce-selected" className="flex items-center gap-2 text-sm font-bold text-[hsl(var(--primary))]"><Volume2 size={16} /> {t("listen")}</button>{(isSpeaking || isPaused) && <button type="button" onClick={togglePause} data-testid="button-pronounce-pause" className="flex items-center gap-1.5 rounded-lg bg-[hsl(var(--card))] px-2.5 py-1 text-xs font-bold text-[hsl(var(--foreground))]">{isPaused ? <Play size={14} /> : <Pause size={14} />} {isPaused ? t("resume") : t("pause")}</button>}</div></div> : <div className="rounded-2xl border border-dashed border-[hsl(var(--border))] p-5 text-sm leading-relaxed text-[hsl(var(--muted-foreground))]">{t("tapWord")}</div>}</div>
      <div className="rounded-[24px] border border-[hsl(var(--border))] bg-[hsl(var(--secondary))] p-5"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-[hsl(var(--card))]"><Headphones size={18} className="text-[hsl(var(--primary))]" /></div><div><h2 className="font-bold">Calm reading voice</h2><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">Tap Listen for a gentle, steady reading pace.</p></div></div><div className="mt-4 flex gap-2"><button type="button" onClick={speakLesson} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[hsl(var(--card))] px-3 py-2 text-sm font-bold text-[hsl(var(--primary))]" data-testid="button-listen-lesson"><Volume2 size={16} /> Listen to whole lesson</button>{(isSpeaking || isPaused) && <button type="button" onClick={togglePause} className="flex items-center justify-center gap-2 rounded-xl bg-[hsl(var(--accent))] px-3 py-2 text-sm font-bold text-[hsl(var(--foreground))]" data-testid="button-sidebar-pause">{isPaused ? <Play size={16} /> : <Pause size={16} />} {isPaused ? t("resume") : t("pause")}</button>}</div></div>
